...
def press(num):
	...
	global expression
	expression = expression + str(num)  #ต่อstrใหม่
	equation.set(expression)    #ตั้งนิพจน์ใหม่
    

def equalpress():
    if expression_field.get()=="":
     tkinter.messagebox.showwarning("warning", "Please only input numbers")
    else: 
        try:
            global expression
            total = str(eval(expression)) #eval การนำ สตริงที่ต้องการมาประมวลผล
